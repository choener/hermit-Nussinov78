...
module HERMIT.Optimization.StreamFusion.ADPfusion (plugin, fixStep, Size.Size(..)) where
    ...
    phase pn $ do
        run $ promoteR
            ...
            $ (bracketR "concatMap -> flatten" concatMapSafe) <+ (unfoldNamesR $ ['VS.concatMap, 'M.concatMap, 'V.concatMap] ++
                [ TH.mkName "forwardFun"  -- forward phase
                , TH.mkName "grammarFun"  -- grammar description
                , TH.mkName "fillFun"     -- filling table(s)
                -- temporary functions
                , TH.mkName "$cmkStream"
                , TH.mkName "$cMkStream"
                , TH.mkName "mkStream"
                , TH.mkName "D:MkStream"
                , TH.mkName "$fMkStreammZSubword"
                , TH.mkName "$fMkStreammPairSubword"
                , TH.mkName "$dMkStream"
                , TH.mkName "$WInner"
                , TH.mkName "$p1MkStream"
                ])
